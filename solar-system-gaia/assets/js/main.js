...
window.addEventListener('error', function(e) {
    ...
    errDiv.innerHTML = '<strong>Aviso do Sistema:</strong> Ocorreu uma interrupção no carregamento tridimensional.<br><small style="opacity:0.8">Por favor, recarregue a visualização para reiniciar os motores orbitais.</small>';
...
const uiTranslations = {
    pt: {
        ...
        zoomLabel: "Zoom:",
        lblEquatorialDiameter: "Diâmetro Equatorial:",
    ...
    en: {
        ...
        speedLabel: "Speed:",
        zoomLabel: "Zoom:",
        lblEquatorialDiameter: "Equatorial Diameter:",
    ...
    es: {
        ...
        btnAbout: "ℹ️ Info",
        btnAboutTitle: "Sobre el Projeto",
        aboutTitle: "Sobre el Projeto",
        aboutText: "Este es un proyecto de construcción de un simulador y enciclopedia con información sobre el Sistema Solar. La herramienta de construcción principal é la ingeniería de prompts con inteligencia artificial generativa. Para sugerencias, envíe sugerencias a:",
        ...
        zoomLabel: "Zoom:",
        lblEquatorialDiameter: "Diámetro Ecuatorial:",
        lblOrbitalPeriod: "Período Orbital:",
...
celestialBodiesData.forEach(data => {
    ...
    if (data.hasRing) {
        ...
        const ringMat = new THREE.MeshStandardMaterial({ color: 0xaaaa88, side: THREE.DoubleSide, transparent: true, opacity: 0.8 });
...
function rebuildOrbitLines() {
    ...
    celestialObjects.forEach(obj => {
        ...
        obj.orbitLine.visible = showOrbits && data.type === 'planet';
...
function updateScene() {
    ...
    celestialObjects.forEach(obj => {
        ...
        } else if (highlightPlanets && data.type === 'planet') {
...
function triggerRaycastClick() {
    ...
    if (intersects.length > 0) {
        ...
        const found = celestialObjects.find(o => o.collisionMesh === hitCollisionMesh);
...
function showInfoBoxData(obj) {
    ...
    document.getElementById('infoBoxDiameter').textContent = typeof data.info.diameter === 'number' ? data.info.diameter.toLocaleString(currentLang === 'en' ? 'en-US' : 'pt-BR') : data.info.diameter;
    document.getElementById('infoBoxDistance').textContent = typeof data.info.distance === 'number' ? data.info.distance.toLocaleString(currentLang === 'en' ? 'en-US' : 'pt-BR') : data.info.distance;
    
    if (typeof data.info.period === 'number') {
        if (data.info.period > 365) {
            const yearsVal = (data.info.period / 365).toFixed(1);
            document.getElementById('infoBoxPeriod').textContent = parseFloat(yearsVal).toLocaleString(currentLang === 'en' ? 'en-US' : 'pt-BR');
            document.getElementById('lblDays').textContent = dict.years;
        } else {
            document.getElementById('infoBoxPeriod').textContent = data.info.period.toLocaleString(currentLang === 'en' ? 'en-US' : 'pt-BR');
    ...
    if (data.info.orbitalSpeed !== undefined && data.info.orbitalSpeed !== 'N/A') {
        document.getElementById('infoBoxSpeed').textContent = data.info.orbitalSpeed.toLocaleString(currentLang === 'en' ? 'en-US' : 'pt-BR');
        document.getElementById('infoBoxSpeedP').style.display = 'block';
    } else document.getElementById('infoBoxSpeedP').style.display = 'none';

    if (data.info.satellites !== undefined && data.info.satellites !== 'N/A') {
        document.getElementById('infoBoxSatellites').textContent = data.info.satellites;
...
showOrbitsCheck.addEventListener('change', e => {
    ...
    celestialObjects.forEach(obj => {
        if (obj.orbitLine) obj.orbitLine.visible = showOrbits && obj.data.type === 'planet';
    });
    moonObjects.forEach(obj => {
